...
const WorkShowCase = memo(({ setLogoColor }: WorkSpaceProps) => {
  ...
  return (
    <motion.div
      key={"home"}
      className={`workshowcase ${absoluteStyle} bg-[#0d0e13] origin-top-left select-none overscroll-none will-change-transform`}
    ...
    >
      ...
      >
        ...
        >
          <div className={`projects ${absoluteStyle} will-change-transform`}>
            {projectDetails.map(({ img, id }) => {
              return (
                ...
                >
                  <div className={`projectWrap ${absoluteStyle} bg-[#0d0e13] will-change-transform`}>
                    <img
                      src={img}
          ...
          <div className="thumbnails relative overflow-hidden pointer-events-none z-[5] h-[50vw] w-[40vw] sm:h-[27vw] sm:w-[22vw]">
            {projectDetails.map(({ img, id }) => {
              return (
                <div
                  key={id}
                ...
                >
                  <img
                    ...
                    className={`thumbnailImage h-full w-full ${absoluteStyle} object-cover will-change-transform`}
                    loading={id <= 2 ? "eager" : "lazy"}
                    decoding="async"
                  />
                </div>
          ...
          <div className="headings absolute left-[8vw] sm:left-[16vw] z-[5] text-white">
            {projectDetails.map(({ title, id, subtitle }) => {
              return (
                id === currentNumber && (
                  <div key={id}>
                    {title.split(" ").map((str) => (
                      <h2 key={str} className="overflow-hidden text-4xl sm:text-6xl tracking-tighter leading-tight">
                        <span className="block">{str}</span>
                      </h2>
                    ))}
                    <p className="overflow-hidden text-base sm:text-xl font-light opacity-80 tracking-tighter">
          ...
          <div className="indicator absolute right-[4vw] bottom-[4vw] z-[9] flex text-lg sm:text-2xl font-light text-white gap-2 overflow-hidden">
          ...
          <motion.button
            className="allProjects absolute flex gap-2 items-center z-[9] left-8 sm:left-14 bottom-8 sm:bottom-14 text-white"
            whileHover="hover"
            initial="initial"
          ...
          >
            ...
            <span className="text-nowrap opacity-80 text-sm sm:text-base">All Projects</span>
          </motion.button>
        </section>
        <AllProjects
